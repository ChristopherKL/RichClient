...
function pkcs1pad2(s,n) {
  ...
  ba[--n] = 0;
  var sc = new rng.SecureRandom();
  var x = new Array();
  while(n > 2) { // random non-zero pad
    x[0] = 0;
    while(x[0] == 0) sc.nextBytes(x);
    ba[--n] = x[0];
  }
  ba[--n] = 2;
  ba[--n] = 0;
...
function RSAGenerate(passwd, B,E) {
  Math.seedrandom(sha256(passwd));
  var sd = new rng.SeededRandom();
  ...
  for(;;) {
    for(;;) {
      this.p = new BigInteger(B-qs,1,sd);
      if(this.p.subtract(BigInteger.ONE).gcd(ee).compareTo(BigInteger.ONE) == 0 && this.p.isProbablePrime(10)) break;
    }
    for(;;) {
      this.q = new BigInteger(qs,1,sd);
